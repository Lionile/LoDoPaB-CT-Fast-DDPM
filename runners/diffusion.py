...
class Diffusion(object):
    ...
    def sg_train(self):
        ...
        elif self.args.dataset=='LODOPAB':  # ADD LODOPAB
            dataset = LODOPAB(self.config.data.train_dataroot, self.config.data.image_size, split='train')
            print('Start training your Fast-DDPM model on LODOPAB dataset.')
    ...
    def sg_sample_fid(self, model):
        ...
        if self.args.dataset=='LDFDCT':
            ...
            sample_dataset = LODOPAB(self.config.data.sample_dataroot, self.config.data.image_size, split='calculate')
            print('Start sampling model on LODOPAB dataset.')
        print('The inference sample type is {}. The scheduler sampling type is {}. The number of involved time steps is {} out of 1000.'.format(self.args.sample_type, self.args.scheduler_type, self.args.timesteps))
